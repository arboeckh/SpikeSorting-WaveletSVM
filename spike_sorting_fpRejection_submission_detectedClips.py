"""Script to asses the sensitivity of the number of spikes detected of algorithm on 
the submission data to threshold changes. comparison vs false positive rejection and
non-false positive rejection"""
import ss 
...
#create spike sorter and train it
spike_sorter = ss.SpikeSorter(**spike_sorter_params)
spike_sorter.train()

#find the number of spkes detected in submission data on old (no fp rejection) and
#new (with fp rejection)
ks = [1,2,3,4,5,6]  #values of threshold k to sweep

#find number of detected peaks for non-fp rejection
for k in ks:
    spike_sorter.params['thK'] = k
    submission_labels, rejected_fn = spike_sorter.eval_new_data(sd, 25e3)
    print(f"old: {k}: {len(submission_labels)}")

#find number of detected peaks with fp rejection
spike_sorter.params['fp_rejection'] = True
spike_sorter.train()
for k in ks:
    spike_sorter.params['thK'] = k
    submission_labels, rejected_fn = spike_sorter.eval_new_data(sd, 25e3)
    print(f"new: {k}: {len(submission_labels)}")
